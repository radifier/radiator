...
void lyra2v2_gpu_hash_32_v3(uint32_t threads, uint32_t startNounce, uint2 *outputHash)
{
...
#endif
	{
		...
		const uint32_t ps1 = (4 * memshift * 3 + 16 * memshift * thread);

		//#pragma unroll 4
		for (int i = 0; i < 4; i++)
		{
			vectype *const s1 = DMatrix + ps1 - 4 * memshift * i;
			for (int j = 0; j < 3; j++)
				s1[j] = state[j];
		...
		const vectype *const shift = DMatrix + (memshift * rowa + 16 * memshift * thread);
