...
static __device__ __forceinline__ void block_tox(const uint32_t *in, uint32_t x[2][2][2][2][2])
{
	x[0][0][0][0][0] ^= in[0];
	x[0][0][0][0][1] ^= in[1];
	x[0][0][0][1][0] ^= in[2];
...
static __device__ __forceinline__ void hash_fromx(uint32_t *out, const uint32_t x[2][2][2][2][2])
{
	int j;
	int k;
	int l;
...
void x11_cubehash512_gpu_hash_64(uint32_t threads, uint32_t startNounce, uint32_t *const __restrict__ g_hash)
{
