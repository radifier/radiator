...
extern void neoscrypt_cpu_hash_k4(int stratum,int thr_id, uint32_t threads, uint32_t startNounce, uint32_t* foundnonce);
extern void neoscrypt_cpu_hash_k4_52(int stratum, int thr_id, int threads, uint32_t startNounce, uint32_t* foundnonce);
...
//	const int throughput = gpus_intensity[thr_id] ? 256 * 64 * gpus_intensity[thr_id] : 256 * 64 * 3.5;
	...
	do {
		...
		if (device_sm[device_map[thr_id]] > 500)
			neoscrypt_cpu_hash_k4_52(stratum, thr_id, throughput, pdata[19], foundNonce);
		else
			neoscrypt_cpu_hash_k4(stratum, thr_id, throughput, pdata[19], foundNonce);
