...
static __forceinline__ __device__ void shift256R2_final(uint32_t *const __restrict__ ret, const uint32_t *const __restrict__ vec4, const uint32_t shift)
{
	uint32_t truc = 0, truc2 = cuda_swab32(vec4[7]), truc3 = 0;
	asm("shf.r.clamp.b32 %0, %1, %2, %3;" : "=r"(truc) : "r"(truc3), "r"(truc2), "r"(shift));
	ret[8] = cuda_swab32(truc);
	truc3 = cuda_swab32(vec4[6]);
	asm("shf.r.clamp.b32 %0, %1, %2, %3;" : "=r"(truc) : "r"(truc2), "r"(truc3), "r"(shift));
	ret[7] = cuda_swab32(truc);
}

static __forceinline__ __device__ uint32_t fastkdf(const uint32_t *const __restrict__ password, uint8_t *const __restrict__ output, const uint32_t *const __restrict__ salt)
{
...
#pragma unroll
		for(int k = 0; k < 9; ++k)
			((uint32_t *)B)[k + (bufidx >> 2)] ^= shifted[k];
...
#pragma unroll
		for(int k = 0; k < 16; k++)
		{
			((uchar4*)(input))[k] = make_uchar4((A + bufidx)[4 * k], (A + bufidx)[4 * k + 1], (A + bufidx)[4 * k + 2], (A + bufidx)[4 * k + 3]);
		}
#pragma unroll
		for(int k = 0; k < 8; k++)
		{
			((uchar4*)(key))[k] = make_uchar4((B + bufidx)[4 * k], (B + bufidx)[4 * k + 1], (B + bufidx)[4 * k + 2], (B + bufidx)[4 * k + 3]);
		}
...
#pragma unroll
		...
		if(salt != NULL)
		{
			shift256R2_final(shifted, input, (bufidx & 3) << 3);
...
#pragma nounroll
		for(int i = 0; i < 64; ++i)
		{
			((uchar4*)output)[i] = make_uchar4(B[(uint8_t)(4 * i + bufidx)], B[(uint8_t)(4 * i + 1 + bufidx)], B[(uint8_t)(4 * i + 2 + bufidx)], B[(uint8_t)(4 * i + 3 + bufidx)]) ^ ((uchar4*)A)[i];
		}
		return 0;
	}
}
...
static __device__ __forceinline__ void neoscrypt_chacha(uint16 *XV)
{
	...
	XV[1] ^= XV[0];
	uint16 temp = chacha_small_parallel_rnd(XV[1]);
	XV[2] ^= temp;
...
static __device__ __forceinline__ void neoscrypt_salsa(uint16 *XV)
{
	...
	XV[1] ^= XV[0];
	uint16 temp = salsa_small_scalar_rnd(XV[1]);
	XV[2] ^= temp;
...
#pragma unroll
	for(int i = 0; i < 5; i++)
		((uint4*)data)[i] = ((uint4 *)c_data)[i];	//ld.local.v4
...
__host__ void neoscrypt_cpu_hash_k4(int stratum, int thr_id, int threads, uint32_t startNounce, const int threadsperblock, uint32_t *result)
{
	...
	neoscrypt_gpu_hash_k0 << <grid, block, 0, gpustream[thr_id] >> >(stratum, startNounce);  //b
	neoscrypt_gpu_hash_k01 << <grid, block, 0, gpustream[thr_id] >> >();  //b
	neoscrypt_gpu_hash_k2 << <grid, block, 0, gpustream[thr_id] >> >();  //a
...
__host__ void neoscrypt_setBlockTarget(int thr_id, uint32_t* pdata, const void *target)
{
	...
	CUDA_SAFE_CALL(cudaMemcpyToSymbolAsync(pTarget, ((uint32_t*)target) + 7, sizeof(uint32_t), 0, cudaMemcpyHostToDevice, gpustream[thr_id]));
	CUDA_SAFE_CALL(cudaMemcpyToSymbolAsync(input_init, input, 16 * sizeof(uint32_t), 0, cudaMemcpyHostToDevice, gpustream[thr_id]));

	CUDA_SAFE_CALL(cudaMemcpyToSymbolAsync(c_data, pdata, 10 * sizeof(uint64_t), 0, cudaMemcpyHostToDevice, gpustream[thr_id]));
}
