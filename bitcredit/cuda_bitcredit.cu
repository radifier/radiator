...
#define TPB 512
__global__ __launch_bounds__(TPB, 2) void bitcredit_gpu_hash(uint32_t threads, uint32_t startNonce, uint32_t* NonceVector)
{
...
void bitcredit_cpu_init(int thr_id, uint32_t threads, uint32_t *hash)
{
	cudaMalloc(&d_BitNonce[thr_id], sizeof(uint32_t));
	cudaMemcpyToSymbolAsync(TheHash, &hash, sizeof(hash), 0, cudaMemcpyHostToDevice, gpustream[thr_id]);
}
